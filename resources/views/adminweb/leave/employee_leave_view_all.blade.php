                      ...
                      <tbody>
                          ...
                          <tr>
                              ...
                              <form action = "{{ route('employee.approve_leave', $leave->id)}}" method="POST" >
                                @csrf
                                <button type="submit" value="{{$leave->id}}" name="employee_id" class = "btn btn-rounded btn-info mb-5">Approve</button>
                              </form>
                              <form action = "{{ route('employee.decline_leave', $leave->id)}}" method="POST" >
                                @csrf
                                <button type="submit" value="{{$leave->id}}" name="employee_id" class = "btn btn-rounded btn-info mb-5">Decline</button>
                              </form>
                              @endif
                              </td>
                          </tr>
